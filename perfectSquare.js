...
function perfectSquare (sq) {// sq is the number that is already squared
 if (Number.isInteger(Math.sqrt(sq)))//if the number is an integer of the sqrt of that number, then do this...
    return (Math.sqrt(sq)+1)*(Math.sqrt(sq)+1);//basically saying (sqrt of 9 is 3)+1 = 4 so 4*4=16
else
    return -1; //if the number does not have a sqrt, then console.log this back
}
